        ...
        for (let y = 0; y < textLines.length; y++) {
            ...
            if (start.x == -1 || end.x == -1) {
                for (let x = 0; x < gridLine.length; x++) {
                    ...
                    if (start.x != -1 && end.x != -1) break;
                }
            }
        }

        // BFS
        queue.push({pos: start, len: 0});
            ...
            for (let dx = -1; dx <= 1; dx++) {
                for (let dy = -1; dy <= 1; dy++) {
                    if (Math.abs(dx) != Math.abs(dy) && withinBounds(curr.pos.x, dx, grid[0].length) && withinBounds(curr.pos.y, dy, grid.length) && !visited[curr.pos.y+dy][curr.pos.x+dx]) {
                        ...
                        if (grid[curr.pos.y][curr.pos.x].charCodeAt(0) >= grid[curr.pos.y+dy][curr.pos.x+dx].charCodeAt(0) - 1) {
...
exports.info = {
    name: "optimalHillClimb",
    desc: "Find the length of the optimal journey from point S to point E!",
    day: 12,
